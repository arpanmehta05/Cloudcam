      ...
      {vmInfo?.public_ip && (
        <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4 text-left space-y-3 select-text">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-foreground">EC2 SSH Connect Command</span>
          </div>
          <p className="text-[10px] text-muted-foreground leading-relaxed">
            The EC2 instance is active. Configure connection details below to generate the SSH command.
          </p>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {["ec2-user", "ubuntu", "admin", "root", "centos", "debian"].map((usr) => (
                <button
                  key={usr}
                  type="button"
